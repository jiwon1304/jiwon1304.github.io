...
- `__cdecl`은 반대로 caller가 callee의 `ret` 이후 stack pointer를 원위치로 돌려놓는다. 가변 인수 함수는 이 방법을 사용해야한다.
callee는 여러 곳에서 호출될텐데, 매번 다른 갯수의 인수와 함께 호출되면서 stack pointer를 얼마나 돌려놔야 하는지 미리 정할 수 없기 때문이다.
