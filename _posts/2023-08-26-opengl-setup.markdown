...
OpenGL을 통해서 혼자 그래픽스를 공부하려는데, 라이브러리라는 것을 처음 쓰려고 하니까 초기 설정하는 것부터 난관이었습니다. 학교에서는 이런걸 가르쳐주지 않고 그냥 빈칸 채워넣기마냥 과제를 주었기 때문에 단순히 코드만 입력하는 것 이외에는 아무것도 때문이었죠. 더 배우고 도전해오라는 뜻이었을까요? 하지만 꼭 배워보고 싶었기에 온갖 시행착오를 겪으면서 찾은 방법을 적어두고자 합니다.\
\
