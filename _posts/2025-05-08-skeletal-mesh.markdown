...
2. `LoadFbxSkeleton(...)`: `eSkeleton` 노드들에 대해 Skeleton 정보 파싱
    1. 각 Bone(Joint)마다 `localBindPose`와 `inverseBindPose` 행렬을 추출

`eSkeleton` 노드들에 대해 Skeleton 정보 파싱

1. `LoadSkinWeights(...)`: `eSkeleton` 노드들에 대해 Skin 정보 파싱
    1. skin - cluster - linkedBone을 통해 controlIndex 별로 연결된 bone을 Map으로 추출
...
보통 FBX 파일에 메쉬 노드는 하나지만 드물게 여러 메쉬 노드가 있을 수 있다. 때문에 모든 노드를 순회하며 `eMesh` 타입 노드를 리스트로 수집하고, 이 리스트에 대해 Mesh, Material 정보를 파싱하게 된다.

각 메쉬마다 대응되는 `UMaterial`과 `FMaterialSubset` 로 머터리얼, 서브셋을 구현하고 있다.
...
- vertex를 bind pose에서 current pose로 변경하는 행렬을 skinning matrix라고 한다.
- matrix가 좌표계를 변환하지 않음
    - 언제나 Model 좌표계를 기준으로 함
    - 대신 vertex의 position이 이동한다
...
FBX SDK는 기본적으로 thread safety를 보장하지 않는다. 따라서 하나의 쓰레드를 생성한 후 그 쓰레드에서만 FBX SDK의 함수를 실행하도록 했다. 또한 데이터를 가공하는 쓰레드와, 바이너리로 저장하는 쓰레드를 생성하여 총 3개의 쓰레드가 FBX 파싱을 작업을 진행하도록 했다. 
...
void FFbxManager::ConvertFunc()
{
    while (!bStopThread)
    {
        // 이전 단계 (파싱)의 쓰레드에서 에서 notify하기 전까지 conditional variable을 기다림 
        ...
        while (!PriorityConvertQueue.IsEmpty() || !ConvertQueue.IsEmpty())
        {
            ...
            if (Queue.Dequeue(FileName))
            {
                ...
                if (!bContaining)
                {
                    // TODO : lock을 두번거는게 아니라 한번걸고 map에 entry를 등록 후 수정해야함.
